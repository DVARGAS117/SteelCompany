...
if (!empty($id) and ($cantidad_tmp > 0) and ($precio_tmp > 0)) {
    $sqlVerificar   = mysqli_query($conexion, "SELECT COUNT(*) AS numrows FROM tmp_ventas WHERE cod_producto='$id' AND session_id='$session_id'");
    ...
    if ($numrows == 0) {
        ...
        $sqlActualizar = mysqli_query($conexion, "UPDATE tmp_ventas SET cantidad_tmp=(cantidad_tmp+$cantidad_tmp) WHERE session_id='$session_id' AND cod_producto='$id'");
    }
}
/*****************************************************/
/*****************************************************/
