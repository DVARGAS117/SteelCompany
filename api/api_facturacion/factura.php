...
$ruc				= $array_emisor['ruc'];
$sqlEmpresa			= mysqli_query($conexion, "SELECT * FROM empresa WHERE ruc='$ruc'");
$row1				= mysqli_fetch_array($sqlEmpresa);
$tipo				= $row1["tipo"];
$clave				= $row1["clave_certificado"];
...
if ($tipo == '1') {
	...
	$pass_firma 	= $clave; //password
	$ruta_ws 		= 'https://e-factura.sunat.gob.pe/ol-ti-itcpfegem/billService?wsdl';
...
if ($tipo == '3') {
	...
	$ruta_firma 	= $url_base . $content_firmas . 'beta/certificado_prueba2.pfx';
	$pass_firma 	= '123456';
...
$resp['ruta_xml'] 			= 'archivos_xml_sunat/cpe_xml/' . $d . '/' . $ruc . '/' . $nombre_archivo . '.XML';
$resp['ruta_cdr'] 			= 'archivos_xml_sunat/cpe_xml/' . $d . '/' . $ruc . '/R-' . $nombre_archivo . '.XML';
$resp['ruta_pdf'] 			= 'controllers/prueba.php?tipo=factura&id=0';
$resp['url_xml'] 			= 'archivos_xml_sunat/cpe_xml/' . $d . '/' . $ruc . '/' . $nombre_archivo . '.XML';
$resp['ruta_ws']			= $ruta_ws;

if ($resp['respuesta'] == "ok") {
	$respuesta			= $resp['respuesta'];
	$url_xml			= $resp['url_xml'];
	$hash_cpe			= $resp['hash_cpe'];
	$hash_cdr			= $resp['hash_cdr'];
	$msj_sunat			= $resp['msj_sunat'];
	$ruta_cdr			= $resp['ruta_cdr'];
	$ruta_ws			= $resp['ruta_ws'];
	$ruc				= $array_emisor['ruc'];
	$doc				= $data['serie_comprobante'] . '-' . $data['numero_comprobante'];
	$cliente			= $data['cliente_numerodocumento'];
	$tipo1				= "Factura";
	$insert = mysqli_query($conexion, "INSERT INTO documentos_electronicos VALUES (NULL,'$ruc','$respuesta','$url_xml','$hash_cpe','$hash_cdr','$msj_sunat','$ruta_cdr','$tipo1','$doc','$cliente','$tipo')");
}
...
function get_array_cabecera($data, $emisor)
{
	$cabecera = array(
		...
		'CODIGO_LOCAL' 				=> $emisor['codigo_local'],
		...
		'RAZON_SOCIAL_EMPRESA' 		=> $emisor['razon_social'],			'CONTACTO_EMPRESA' 			=> "",
...
function get_array_emisor($data)
{
	...
	$emisor['codigo_local'] 			= (isset($data_emisor['codigo_local'])) ? $data_emisor['codigo_local'] : '';
